...
Ported from the Arduino Library Inkplate5V2.h / Inkplate5V2.cpp:
...
/**
 * @brief Low level e-Ink display for Inkplate 5 V2
 *
 * This class implements the low level methods required to control
 * and access the e-ink display of the InkPlate 5 V2 device.
 *
 * Key differences from Inkplate 6:
 *   - Resolution: 1280 x 720 (vs 800 x 600)
 *   - 9-phase waveform for 3-bit grayscale (vs 8-phase)
 *   - Row-flipped scan direction (forward per row, reversed within each row)
 *   - Fewer cleaning cycles and refresh repetitions for faster updates
 *   - Uses I2S DMA for data transfer
 *
...
class EInk5V2 : public EInk, NonCopyable {
public:
  ...
  private:
    ...
    static const uint8_t  WAVEFORM_3BIT[8][9]; // 9 phases (last element is terminator 0)
